...
function Dashboard() {
  ...
  return <div className="page">
    <PageHeader eyebrow="AQUAPHOR OPERATIONS" title="AQUAPHOR Customer Command Center" subtitle="One verified operating system for every customer, contact, application, installation and service obligation." action={<><Link className="button button-secondary" to="/intelligence"><Sparkles size={16} /> Ask AI</Link><Link className="button button-primary" to="/applications/new"><Plus size={16} /> New application</Link></>} />
    <section className="intelligence-banner"><div><span className="banner-kicker"><Activity size={15} /> LIVE CUSTOMER GRAPH</span><h2>From application to lifetime service - one connected customer record.</h2><p>Search any customer, inspect the verified 360 dossier, prioritise overdue work and draft precise follow-up from the underlying data.</p></div><div className="banner-orbit"><ALogo /><i /><i /><i /></div></section>
...
function ClientsPage() {
  ...
  function exportCsv() {
    const rows = [['Client','Email','Phone','City','Application / Customer Date','VAT','Source','Contacts','Tasks','Applications','Next Service','Quality'], ...(data?.items || []).map((r) => [displayName(r),r.email||'',r.phone||'',r.city||'',r.application_date||r.legacy_created_at||r.created_at||'',r.vat||'',r.origin||'',r.contact_count||0,r.task_count||0,r.application_count||0,r.next_service_due||'',r.data_quality_score||''])];
